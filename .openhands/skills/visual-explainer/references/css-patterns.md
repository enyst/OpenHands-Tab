...
function applyTransform(target, zoom, panX, panY) {
  ...
  target.dataset.panX = String(panX);
  target.dataset.panY = String(panY);
  // Scale first, then translate by panX/panY in screen pixels.
  target.style.transform = 'scale(' + zoom + ') translate(' + (panX / zoom) + 'px, ' + (panY / zoom) + 'px)';
}
...
function zoomDiagram(btn, factor) {
  ...
  var panX = parseFloat(target.dataset.panX || '0');
  var panY = parseFloat(target.dataset.panY || '0');
  var next = Math.min(Math.max(current * factor, 0.3), 5);
  applyTransform(target, next, panX, panY);
...
window.addEventListener('mousemove', function(e) {
  ...
  var target = activeWrap.querySelector('.mermaid');
  var zoom = parseFloat(target.dataset.zoom || '1');
  applyTransform(target, zoom, startPanX + (e.clientX - startX), startPanY + (e.clientY - startY));
});

window.addEventListener('mouseup', function() {
  if (!activeWrap) return;
  activeWrap.classList.remove('is-panning');
  activeWrap = null;
});
...
document.querySelectorAll('.mermaid-wrap').forEach(function(wrap) {
  ...
  wrap.addEventListener('mousedown', function(e) {
    ...
    startX = e.clientX;
    startY = e.clientY;
    startPanX = parseFloat(target.dataset.panX || '0');
    startPanY = parseFloat(target.dataset.panY || '0');
  });
});
```
