...
When the UI shows `Context: N tokens` in the toolbar or history list, the value comes from the last LLM response usage (`state.values.llm_usage.inputTokens` when available). When `llm_usage` is missing, the UI falls back to the most recent prompt token count in `state.values.stats` (lastTokenUsage or accumulated usage). These are provider‑reported **prompt tokens**, so they include everything sent in the request: system prompt, tool definitions, user message, and any extended context appended to the user message.
