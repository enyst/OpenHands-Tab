...
type NormalizedLlmProvider = 'openai' | 'openrouter' | 'litellm_proxy' | 'anthropic';

const normalizeLlmProvider = (value: unknown): NormalizedLlmProvider | undefined => {
  const trimmed = typeof value === 'string' ? value.trim() : '';
...
const FALLBACK_LLM_PROVIDER: NormalizedLlmProvider = 'anthropic';
const FALLBACK_AGENT_ENABLE_SECURITY_ANALYZER = false;
const FALLBACK_AGENT_DEBUG = false;
...
export class SettingsManager {
  ...
  async get(): Promise<OpenHandsSettings> {
    ...
    const explicitBaseUrlRaw = this.adapter.getExplicit('openhands.llm.baseUrl');
    const explicitBaseUrl = normalizeNonEmptyString(typeof explicitBaseUrlRaw === 'string' ? explicitBaseUrlRaw : undefined);
    const explicitProvider = normalizeLlmProvider(this.adapter.getExplicit('openhands.llm.provider'));
    const provider: NormalizedLlmProvider | undefined = isRemote
      ? explicitProvider
      : explicitProvider ?? (explicitBaseUrl ? undefined : FALLBACK_LLM_PROVIDER);
    ...
    const agentEnableSecurityAnalyzerRaw = this.adapter.get('openhands.agent.enableSecurityAnalyzer');
    const agentDebugRaw = this.adapter.get('openhands.agent.debug');
    const enableSecurityAnalyzer = typeof agentEnableSecurityAnalyzerRaw === 'boolean'
      ? agentEnableSecurityAnalyzerRaw
      : FALLBACK_AGENT_ENABLE_SECURITY_ANALYZER;
    const debug = typeof agentDebugRaw === 'boolean' ? agentDebugRaw : FALLBACK_AGENT_DEBUG;
    const agent: AgentSettings = { enableSecurityAnalyzer, debug };
